...
public final class AquacultureCompat {
	...
	private static final ResourceLocation ROD_INVENTORY_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "rod_inventory");
	private static final ResourceLocation NOTE_CATCH_SOUND_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "bobber_note_catch");
	private static final ResourceLocation NEPTUNIUM_ROD_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "neptunium_fishing_rod");
	...
	public static Tackle read(ItemStack rod) {
		...
		ResourceLocation fitted = hookId(slots);
	...
	@Nullable
	private static ResourceLocation hookId(List<ItemStack> slots) {
		if (slots.size() <= HOOK_SLOT || slots.get(HOOK_SLOT).isEmpty()) return null;
		ResourceLocation id = BuiltInRegistries.ITEM.getKey(slots.get(HOOK_SLOT).getItem());
	...
	private static TackleLook look(List<ItemStack> slots, @Nullable ResourceLocation fittedHook) {
	...
	private static ResourceLocation texture(String path) {
		return ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/entity/rod/" + path + ".png");
	}
	...
	private static DataComponentType<ItemContainerContents> rodInventoryType() {
		DataComponentType<?> type = BuiltInRegistries.DATA_COMPONENT_TYPE.get(ROD_INVENTORY_ID);
	...
	private static ResourceKey<LootTable> lootTable(String path) {
		return ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.fromNamespaceAndPath(MOD_ID, path));
	}
}
