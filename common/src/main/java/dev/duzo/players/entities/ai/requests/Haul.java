...
public record Haul(UUID quartermaster, ResourceLocation item, int baseline, long since, int wanted) {
	...
	public static Haul of(AIState state) {
		...
		if (!params.contains(TAG)) return null;
		CompoundTag tag = params.getCompound(TAG);
		if (tag.isEmpty()) return null;
		int[] raw = tag.contains("Qm") ? tag.getIntArray("Qm") : null;
		if (raw == null || raw.length != 4) return null;
		String itemStr = tag.contains("Item") ? tag.getString("Item") : "";
		ResourceLocation item = ResourceLocation.tryParse(itemStr);
		if (item == null) return null;
		int base = tag.contains("Base") ? tag.getInt("Base") : 0;
		long since = tag.contains("Since") ? tag.getLong("Since") : 0L;
		int want = tag.contains("Want") ? tag.getInt("Want") : 0;
		return new Haul(UUIDUtil.uuidFromIntArray(raw), item, base, since, want);
	}
