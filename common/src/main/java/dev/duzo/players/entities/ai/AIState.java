...
public final class AIState {
	...
	private static ResourceLocation readJobId(CompoundTag tag) {
		...
		// contains, not getIntOr with a 0 default: an absent Job entry must miss the frozen table
		// rather than be handed ordinal 0 (none) by the accessor's own default.
		ResourceLocation migrated = LegacyJobIds.byOrdinal(tag.contains("Job") ? tag.getIntOr("Job", -1) : -1);
