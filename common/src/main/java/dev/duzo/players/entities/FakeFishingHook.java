...
public class FakeFishingHook extends Projectile {
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(DATA_ROD, ItemStack.EMPTY);
		builder.define(DATA_LAVA_PROOF, false);
	}
	...
	public void tick() {
		...
		if (!FPJobs.is(ai.jobId(), FPJobs.FISHERMAN) || !ai.running()) { discard(); return; }
	...
	protected void readAdditionalSaveData(CompoundTag tag) {
		...
		try { state = State.valueOf(tag.getStringOr("HookState", "FLYING")); } catch (IllegalArgumentException ignored) {}
		life = tag.getIntOr("Life", 0);
		setLavaProof(tag.getBooleanOr("LavaProof", false));
	}
}
