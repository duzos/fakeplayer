...
public class AISubMenuScreen extends Screen {
	...
	private void cycleJob() {
		List<Identifier> order = FPJobs.selectableOrder();
		...
		Identifier next = order.get((current + 1) % order.size());
	...
	private String defaultFilterFor(Identifier jobId) {
		return FPJobs.is(jobId, FPJobs.COURIER) ? "*" : "c:ores";
	}
