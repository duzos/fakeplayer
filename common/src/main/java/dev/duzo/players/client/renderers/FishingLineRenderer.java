...
/**
 * Draws every {@link FakeFishingHook}: a bobber as a camera-facing quad plus a line from the owning
 * fake's hand to the bobber. Runs from the same world-render hook that drives
 * {@link SessionItemRenderer}, so it stays off the entity submit-node pipeline.
 *
 * <p>A rod with modded tackle fitted reports a {@link TackleLook}, which swaps in that mod's bobber
 * and hook textures and its dyed line colour; anything else draws the vanilla bobber.
 */
public final class FishingLineRenderer {
	...
	public static void render(PoseStack pose) {
		...
		for (Entity e : level.entitiesForRendering()) {
			...
			if (look.isPlain()) {
				drawQuad(pose, buffers.getBuffer(RenderTypes.entityCutoutNoCull(BOBBER_TEXTURE)), camRot, bobber, 0xFFFFFFFF);
			} else {
				// Same layering their own renderer uses: bobber background, a tinted overlay, then the hook.
				if (look.bobberTexture() != null)
					drawQuad(pose, buffers.getBuffer(RenderTypes.entityCutoutNoCull(look.bobberTexture())), camRot, bobber, 0xFFFFFFFF);
				if (look.bobberOverlay() != null)
					drawQuad(pose, buffers.getBuffer(RenderTypes.entityCutoutNoCull(look.bobberOverlay())), camRot, bobber,
							look.bobberColor() < 0 ? 0xFFFFFFFF : look.bobberColor());
				if (look.hookTexture() != null)
					drawQuad(pose, buffers.getBuffer(RenderTypes.entityCutoutNoCull(look.hookTexture())), camRot, bobber, 0xFFFFFFFF);
			}
		}
