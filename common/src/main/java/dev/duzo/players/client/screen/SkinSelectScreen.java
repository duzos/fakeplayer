...
public class SkinSelectScreen extends Screen {
	...
	public SkinSelectScreen(FakePlayerEntity target) {
		...
		// Never added to a level, so it has no id, and the item model resolver reads getId() while extracting
		// the render state. Zero is the unassigned sentinel that throws; real entity ids are positive.
		this.render.setId(-1);
