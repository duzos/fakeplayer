...
public record SetJobPacketC2S(int id, int jobOrdinal) {
	...
	public static void handle(PacketContext<SetJobPacketC2S> ctx) {
		...
		// let the old executor's pause path run and persist before its state is wiped out below,
		// otherwise switching jobs mid-craft skips whatever cleanup the old job needed on pause.
