 ...
 */
public record RequestKey(UUID requester, RequesterKind kind, Identifier jobId, Identifier item) {
	...
	public static RequestKey fromNbt(CompoundTag tag) {
		...
		Identifier jobId = readJobId(tag);
	...
	private static Identifier readJobId(CompoundTag tag) {
		...
		Identifier migrated = LegacyJobIds.byName(tag.getStringOr("Job", ""));
