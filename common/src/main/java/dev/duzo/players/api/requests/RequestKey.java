...
public record RequestKey(UUID requester, RequesterKind kind, Job job, ResourceLocation item) {
	...
	public static RequestKey fromNbt(CompoundTag tag) {
		int[] raw = tag.contains("Requester") ? tag.getIntArray("Requester") : null;
		...
		Job job;
		try {
			String jobName = tag.contains("Job") ? tag.getString("Job") : Job.NONE.name();
			job = Job.valueOf(jobName);
		} catch (IllegalArgumentException e) {
			job = Job.NONE;
		}
		String kindName = tag.contains("Kind") ? tag.getString("Kind") : RequesterKind.FAKE.name();
		return new RequestKey(UUIDUtil.uuidFromIntArray(raw),
				RequesterKind.byName(kindName, RequesterKind.FAKE),
				job, item);
	}
}
